...
fun TodoItemEntryInput( onItemComplete: ( TodoItem ) -> Unit ) {
    // onItemComplete is an event that will fire when an item is completed by the user
    ...
    TodoItemInput(
        ...
        // Since our slot is the last parameter we can supply it in a trailing lambda
...
fun TodoItemInput(
    ...
    Column {
        Row(
            ...
            // We are replacing the button directly placed, with a slot configuration
            /*TodoEditButton(
                text = "Add",
                onClick = submit, // pass the submit callback to TodoEditButton
                modifier = Modifier.align(Alignment.CenterVertically),
                enabled = text.isNotBlank() // Enable if text is not blank
            )*/ // So it has been remove, in favor of:

            Spacer( modifier = Modifier.width( 8.dp ) )

            Box( Modifier.align( Alignment.CenterVertically ) ) { buttonSlot() }

            // And that is how we make use of our slot :)
        }
        if (iconsVisible)
            //Row { // Our AnimatedIconRow includes a Row, go figure :)
                AnimatedIconRow(
                    icon = icon,
                    onIconChange = onIconChange,
